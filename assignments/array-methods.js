...
// The total needs to be in the array??
...
// Problem 1
// Number of people from each company
let company = {}
runners.forEach((runner) => {
    company[runner.company_name] = (company[runner.company_name] || 0) + 1
})
// console.log(company)

// Problem 2
// Does email and Company match
let emails = []

runners.forEach((runner) => {
    emails.push({
        provider: runner.email.slice(runner.email.indexOf('@')),
...
// Problem 3
// Company to donation correlation 

const getCompaniesDonation = () => {
    let res = {}
    runners.map((runner) => {
        return {company: runner.company_name, donation: runner.donation}
    }).forEach((runner) => {
        res[runner.company] = (res[runner.company] || 0) + runner.donation
    })
    return res;
    // .filter((res) => {
    //     if (res.donation > min)
...
let donation_company = getCompaniesDonation(100)

// console.log(donation_company)